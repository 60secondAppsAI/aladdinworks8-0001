        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Capacity Sensors</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalCapacitySensors = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalCapacitySensors">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add CapacitySensor</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="CapacitySensorId" type="text" placeholder="Enter CapacitySensorId" v-model="capacitySensorToAdd.capacitySensorId"></base-input>
  <base-input label="Capacity" type="text" placeholder="Enter Capacity" v-model="capacitySensorToAdd.capacity"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="capacitySensors" :row-key="record => record.CapacitySensorId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <CapacitySensorPictureView :capacitySensors="capacitySensors" />
          </div>
...
import CapacitySensorPictureView from './CapacitySensorPictureView.vue';


const capacitySensorsColumns = [
  "capacitySensorId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  components: {
    ...
    Pagination,
    CapacitySensorPictureView
  },

  data() {
    return {
      modalCapacitySensors: false,
        isTableView: true,

      columns: [
        {
          title: 'Capacity Sensor Id',
		dataIndex: 'capacitySensorId',
          visible: true,
          scopedSlots: { customRender: 'capacitySensorId' },
          sorter: true
          //sorter: (a, b) => a.capacitySensorId - b.capacitySensorId,
          //sorter: (a, b) => a.capacitySensorId.localeCompare(b.capacitySensorId),
        },
        {
          title: 'Capacity',
		dataIndex: 'capacity',
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} capacitySensors`,
      },

      capacitySensors: [],
      capacitySensorToAdd: {},

      capacitySensorsTable: {
        columns: [...capacitySensorsColumns],
        data: [],
      },

      // New properties for sorting and searching
      sortBy: 'capacitySensorId',           // Column to sort by
  ...
  methods: {
    ...
    async renderCapacitySensorsTable() {
      ...
      let capacitySensorsTableData = [];
      for (let i = 0; i < this.capacitySensors.length; i++) {
        capacitySensorsTableData.push({
          id: i,
          capacitySensorId: this.capacitySensors[i].capacitySensorId,
          year: this.capacitySensors[i].year,
          date: this.capacitySensors[i].date,
          competitionId: this.capacitySensors[i].competitionId,
          capacitySensorId: this.capacitySensors[i].capacitySensorId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-capacity-sensors',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForCapacitySensorsChanged', this.searchQuery);
		//this.renderCapacitySensorsTable();
    },
	...
	async handleAddSubmitted() {
      this.modalCapacitySensors = false;

      const currentDate = new Date().getTime();
      this.capacitySensorToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.capacitySensorToAdd);
      console.log(jsonData);
      
      const res = await CapacitySensorService.addCapacitySensor(jsonData);
