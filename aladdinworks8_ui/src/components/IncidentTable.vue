...
<template>
        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalIncidents = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalIncidents">
                <template slot="header">
  ...
  <base-input label="ReportedDate" type="text" placeholder="Enter ReportedDate" v-model="incidentToAdd.reportedDate"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="incidents" :row-key="record => record.IncidentId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <IncidentPictureView :incidents="incidents" />
          </div>
...
import IncidentPictureView from './IncidentPictureView.vue';
    ...
    return {
      ...
      columns: [
		...
		dataIndex: 'incidentId',
          visible: true,
          scopedSlots: { customRender: 'incidentId' },
          sorter: true
		...
		dataIndex: 'reportedDate',
          visible: true,
          scopedSlots: { customRender: 'reportedDate' },
          sorter: true
          //sorter: (a, b) => a.reportedDate - b.reportedDate,
      ...
      sortBy: 'incidentId',           // Column to sort by
  ...
  methods: {
    ...
    async renderIncidentsTable() {
      ...
      for (let i = 0; i < this.incidents.length; i++) {
        incidentsTableData.push({
          id: i,
          incidentId: this.incidents[i].incidentId,
          year: this.incidents[i].year,
          date: this.incidents[i].date,
          competitionId: this.incidents[i].competitionId,
          incidentId: this.incidents[i].incidentId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-incidents',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForIncidentsChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      ...
      this.incidentToAdd.created = currentDate;
