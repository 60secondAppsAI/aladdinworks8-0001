        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalServiceContracts = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalServiceContracts">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add ServiceContract</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="ServiceContractId" type="text" placeholder="Enter ServiceContractId" v-model="serviceContractToAdd.serviceContractId"></base-input>
  <base-input label="Supplier" type="text" placeholder="Enter Supplier" v-model="serviceContractToAdd.supplier"></base-input>
  <base-input label="ValidityPeriod" type="text" placeholder="Enter ValidityPeriod" v-model="serviceContractToAdd.validityPeriod"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="serviceContracts" :row-key="record => record.ServiceContractId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <ServiceContractPictureView :serviceContracts="serviceContracts" />
          </div>
...
import ServiceContractPictureView from './ServiceContractPictureView.vue';


const serviceContractsColumns = [
  "serviceContractId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  components: {
    ...
    Pagination,
    ServiceContractPictureView
  },

  data() {
    return {
      modalServiceContracts: false,
        isTableView: true,
      ...
      columns: [
		...
		dataIndex: 'serviceContractId',
          visible: true,
          scopedSlots: { customRender: 'serviceContractId' },
          sorter: true
          //sorter: (a, b) => a.serviceContractId - b.serviceContractId,
          //sorter: (a, b) => a.serviceContractId.localeCompare(b.serviceContractId),
        },
        {
          title: 'Supplier',
		dataIndex: 'supplier',
		...
		dataIndex: 'validityPeriod',
          ...
          //sorter: (a, b) => a.validityPeriod.localeCompare(b.validityPeriod),
        },
      ],
      pagination: {
        current: 1,
        ...
        showTotal: (total) => `Total ${total} serviceContracts`,
      },

      serviceContracts: [],
      serviceContractToAdd: {},
      ...
      sortBy: 'serviceContractId',           // Column to sort by
  ...
  methods: {
    ...
    async renderServiceContractsTable() {
      ...
      let serviceContractsTableData = [];
      for (let i = 0; i < this.serviceContracts.length; i++) {
        serviceContractsTableData.push({
          id: i,
          serviceContractId: this.serviceContracts[i].serviceContractId,
          year: this.serviceContracts[i].year,
          date: this.serviceContracts[i].date,
          competitionId: this.serviceContracts[i].competitionId,
          serviceContractId: this.serviceContracts[i].serviceContractId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-service-contracts',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForServiceContractsChanged', this.searchQuery);
		//this.renderServiceContractsTable();
    },
	...
	async handleAddSubmitted() {
      this.modalServiceContracts = false;

      const currentDate = new Date().getTime();
      this.serviceContractToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.serviceContractToAdd);
      console.log(jsonData);
      
      const res = await ServiceContractService.addServiceContract(jsonData);
